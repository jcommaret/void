...
export const subTextMdOfProviderName = (providerName: ProviderName): string => {
	...
	if (providerName === 'mistral') return 'Get your [API Key here](https://console.mistral.ai/api-keys). In the Agents Window, requests are throttled to **1/sec** by default to stay under Mistral rate limits — adjust with the `chat.agentHost.mistralAgent.requestsPerSecond` setting (reload the window to apply).'
	if (providerName === 'openAICompatible') return `Use any provider that's OpenAI-compatible (use this for llama.cpp and more).`
	if (providerName === 'googleVertex') return 'You must authenticate before using Vertex with Kodia. Read more about endpoints [here](https://cloud.google.com/vertex-ai/generative-ai/docs/multimodal/call-vertex-using-openai-library), and regions [here](https://cloud.google.com/vertex-ai/docs/general/locations#available-regions).'
